...
Podspawn logs session events (create, connect, disconnect, destroy) to the audit log at `/var/log/podspawn/audit.log`. Each line is a structured JSON entry with timestamp, user, project, container ID, and event type.
...
| Audit log | `/var/log/podspawn/audit.log` | Session lifecycle events (structured JSON) |
