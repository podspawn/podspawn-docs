...
<Banner>podspawn is hardened by default -- cap-drop ALL, no-new-privileges, PID limits. This guide covers additional hardening options.</Banner>
...
<Accordions>
<Accordion title="Non-root containers">

Containers never run as root. Podspawn creates a non-root user inside every container at startup, matching your OS username (local mode) or SSH username (server mode). The user has:

- **UID 1000** and a home directory at `/home/<username>`
- **Passwordless sudo** for when you actually need root (e.g., `apt-get install`)
- **Shell prompt** showing `username@machinename` (e.g., `karthik@dev:~$`) instead of `root@containerid`

User creation is idempotent. Reconnecting to an existing container skips user setup. This works across Ubuntu, Debian, Alpine, Fedora, and Rocky Linux. On Alpine, `shadow` and `sudo` are installed automatically since they are not included in the base image.

Lifecycle hooks (`on_create`, `on_start`) and dotfiles scripts run as the non-root user. Use `sudo` for operations that need root privileges.
...
<TypeTable type={{
  CHOWN: { type: '"cap"', description: 'Changing file ownership (e.g., chown in setup scripts)' },
  SETUID: { type: '"cap"', description: 'Switching users inside the container' },
  SETGID: { type: '"cap"', description: 'Group permission operations' },
  DAC_OVERRIDE: { type: '"cap"', description: 'Reading/writing files regardless of permissions (needed for dev workflows)' },
  FOWNER: { type: '"cap"', description: 'Operations on files regardless of ownership' },
  NET_BIND_SERVICE: { type: '"cap"', description: 'Binding to ports below 1024' },
}} />
...
<Accordion title="Per-user network isolation">
...
<Accordion title="auth-keys crash safety">
...
<Accordions>
<Accordion title="Customizing capabilities">
...
security:
  ...
  tmpfs:
    ...
    /home/<username>: "rw,exec,nosuid,size=2g"
```
...
| `/home/<username>` | `rw,exec,nosuid,size=2g` | User's home directory. `exec` is needed for development tools. |

The `nosuid` flag on both prevents setuid/setgid exploitation.

<Callout type="warn">
With a read-only root filesystem, `apt-get install` and similar package managers will fail. All packages must be pre-installed in the image or Podfile. This is an intentional trade-off for stronger isolation.
</Callout>

</Accordion>
<Accordion title="PID limit tuning">
...
<Accordion title="Network isolation verification">
...
<TypeTable type={{
  connect: { type: '"event"', description: 'User, project, container name, connection count' },
  disconnect: { type: '"event"', description: 'User, project, container name, remaining connections' },
  container_create: { type: '"event"', description: 'User, project, container name, image' },
  container_destroy: { type: '"event"', description: 'User, project, container name, reason' },
  command: { type: '"event"', description: 'User, project, SSH_ORIGINAL_COMMAND value' },
}} />
...
<Accordion title="Server setup safety">
...
<Callout type="warn">
If podspawn ever malfunctions, the emergency key at `/etc/podspawn/emergency.keys` provides a fallback SSH access path that is independent of podspawn. Do not delete this file.
</Callout>
...
<Cards>
  ...
  <Card title="Networking" href="/docs/concepts/networking" />
