...
If SSH asks for a password instead of using your key, something is intercepting or misconfiguring the auth flow. Walk through these checks in order.

### Check 1: Is something else on port 22?

On macOS, OrbStack Linux machines bind to port 22 and intercept SSH connections before they reach your system's sshd. Podspawn's `AuthorizedKeysCommand` never runs because OrbStack handles the connection.

```bash
sudo lsof -i :22 -P -n | head -5
```

If you see something other than `/usr/sbin/sshd`, that's your problem. Stop the conflicting service (e.g., `orb stop --all` for OrbStack Linux machines) and verify port 22 is free before retrying. For local development, you can skip SSH entirely and use `podspawn shell` instead.
...
The directory needs to be readable and traversable by `nobody` (mode `0755`). Key files inside need to be readable (mode `0644` or `0600` with appropriate ownership).
