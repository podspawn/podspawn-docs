...
<Callout type="info">
Companion services share the session lifecycle. When the session is destroyed (grace period expires, max lifetime hit, or destroy-on-disconnect), all service containers are stopped and the network is removed. See [Session Lifecycle](/docs/concepts/session-lifecycle) for the full cleanup flow.
</Callout>
