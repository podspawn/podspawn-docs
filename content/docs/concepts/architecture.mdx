...
User exits
    |
    +-- ephemeral (grace-period / destroy-on-disconnect)
    |   --> grace period --> container destroyed
    |
    +-- persistent
...
- **grace-period** -- the container lingers for a configurable window (default 10 minutes), then gets destroyed. Reconnecting within the window reattaches to the same container.
- **destroy-on-disconnect** -- the container is removed immediately when the SSH session ends.
- **persistent** -- the container stays alive indefinitely until explicitly destroyed.
