...
1. Rejects usernames containing `/` or `..` (path traversal protection)
2. Opens `/etc/podspawn/keys/<username>`
3. If the file doesn't exist, returns nothing -- zero keys, zero output
4. For each key line, wraps it with the forced command directive and writes to stdout
...
```
sshd calls: podspawn auth-keys bob
    |
    v
/etc/podspawn/keys/bob exists?
    +-- NO  --> return nothing (empty stdout, exit 0)
    |           sshd falls through to ~/.ssh/authorized_keys
    |           bob logs in normally, no container
    |
    +-- YES --> return keys with command= directive
                sshd matches key, forces podspawn spawn
```
