...
flowchart TD
    Start(( )) --> Running
    Running -->|"disconnect (grace mode)"| GracePeriod
    Running -->|"disconnect (destroy mode)"| Destroyed
    Running -->|"disconnect (persistent)"| Persistent["Persistent — container stays alive"]
    GracePeriod -->|reconnect| Running
    GracePeriod -->|grace expires| Destroyed
    Persistent -->|reconnect| Running
    Destroyed --> End(( ))
```
