...
1. **Server default** -- `session.mode` in `/etc/podspawn/config.yaml`
2. **User override** -- `session.mode` in `/etc/podspawn/users/<username>.yaml`
3. **Project config** -- `session.mode` in the project's Podfile
...
| Mode | Grace period | Max lifetime | Home bind mount | Use case |
|---|---|---|---|---|
| `grace-period` (default) | Configurable, default 60s | Enforced, default 8h | No | Human developers -- survives network blips |
| `destroy-on-disconnect` | Zero | Enforced, default 8h | No | CI, AI agents -- immediate cleanup, no billing surprises |
| `persistent` | None, container stays alive | Exempt | Yes, from `/var/lib/podspawn/homes/<user>/` | Long-lived dev environments -- files survive container recreation |
