...
1. Clones the repository to the local project cache (`/var/lib/podspawn/projects/<name>` in server mode, `~/.podspawn/projects/<name>` in local mode)
...
5. Saves the project config to the active projects registry (`/etc/podspawn/projects.yaml` in server mode, `~/.podspawn/projects.yaml` in local mode)
