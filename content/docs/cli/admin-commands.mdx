...
5. With `--purge`, removes the user's persistent home directory at `<homes_dir>/<username>`
6. Without `--purge`, prints a notice if a persistent home directory exists so you can decide whether to keep it
...
| Mode | When | Checks |
|------|------|--------|
| **Local** | `/etc/podspawn/config.yaml` does not exist | Docker daemon, Docker socket, state directory, lock directory, disk space, default image (6 checks) |
| **Server** | `/etc/podspawn/config.yaml` exists | All local checks plus OpenSSH version, sshd config, AuthorizedKeysCommand, podspawn config dir, key directory (11 checks) |
...
#### AuthorizedKeysCommand (server only)
...
#### podspawn config dir (server only)
