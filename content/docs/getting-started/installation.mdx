...
**Client:**
- Any SSH client on any OS
- Optional: the `podspawn` binary for `.pod` namespace routing

<Callout type="info">
OpenSSH 7.4 shipped in December 2016. Check your version with `ssh -V`.
</Callout>
...
| Path | Purpose |
|---|---|
| `/usr/local/bin/podspawn` | The binary (server and client are the same) |
