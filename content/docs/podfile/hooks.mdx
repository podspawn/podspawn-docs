...
| | `extra_commands` | `on_create` |
|---|---|---|
| **When** | Image build time | Container start (first connection) |
| **Runs as** | root | Non-root user (sudo available) |
| **Cached** | Yes, in Docker layer cache | No, runs every time a new container is created |
| **Use for** | System packages, native libraries, build tools | Project dependencies, database setup, migrations |
