...
<Callout>
Both hooks run as the non-root container user (not root). Use `sudo` for operations that need root privileges. A non-zero exit code is logged as a warning but does not prevent the session from starting. The container remains usable even if hooks fail.
</Callout>
...
The `dotfiles` field clones a git repository into `~/dotfiles` (i.e., `/home/<username>/dotfiles`) inside the container and optionally runs an install script.
...
| `install` | `string` | Command to run after cloning. Executed with `sh -c` from the `~/dotfiles` directory. |

The install command runs as the non-root container user:

```sh
cd ~/dotfiles && ./install.sh
```

Use `sudo` inside your install script for operations that need root (e.g., `sudo apt-get install`).
