...
If the container already exists when you connect, `on_create` does not run again. For local branch workspaces, stopping and restarting the machine also skips `on_create` once initialization succeeded, because podspawn persists the initialized workspace separately from the container. If initialization failed earlier, the next `create` or `shell` retries `on_create`, so the hook should be idempotent. To force a clean rerun after a successful initialization, remove the workspace and create a new machine.
</Tab>
...
- **Exit code:** `on_create` is fatal in local machine creation. If it exits non-zero, podspawn tears down the failed container and returns an error. For local named project machines, the checkout is left on disk so you can inspect and fix it before retrying. `on_start` remains non-fatal and logs a warning on failure.
...
In local `podspawn create` flows there is usually no forwarded `SSH_AUTH_SOCK`, so hooks or dotfiles installs that need private SSH git access must rely on host-side git auth, HTTPS tokens, or another credential path.
