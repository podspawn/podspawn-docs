...
<Tabs items={['String', 'Array', 'Object (map)']}>
<Tab value="String">
Used as-is:
...
<Callout>
Converted hooks (`postCreateCommand` and `postStartCommand`) run as the non-root container user with `sudo` available, the same as native Podfile hooks. See [Hooks & Lifecycle](/docs/podfile/hooks) for details on execution context and error handling.
</Callout>

## Example

<Tabs items={['devcontainer.json', 'Equivalent podfile.yaml']}>
<Tab value="devcontainer.json">
```json
{
...
<Tab value="Equivalent podfile.yaml">
```yaml
