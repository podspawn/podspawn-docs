...
The first match wins. If neither is found, podspawn falls back to a `devcontainer.json` if one exists (see [devcontainer fallback](/docs/podfile/devcontainer)), or uses the server defaults.
