...
# Default git branch for local project-backed machines
# branch: main
...
| `branch` | `string` | no | repo default branch | Default branch for `podspawn create/run --project` when `--branch` is omitted. |
...
<Accordions>
<Accordion title="devcontainer.json fallback">
